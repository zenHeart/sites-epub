...
> 上传文件用于文本内容抽取、图片理解或视频理解，支持 pdf、doc、txt 等文本类格式。

<Note>
  单文件大小上限为 100 MiB，超限时返回文件过大错误。文件个数不再受限，改为按组织总存储量限制，默认 10 GiB。文件解析服务限时免费，请求高峰期平台可能会有限流策略。
</Note>

<Accordion title="支持的格式">
  文件接口支持以下格式：`.pdf`、`.txt`、`.csv`、`.doc`、`.docx`、`.xls`、`.xlsx`、`.ppt`、`.pptx`、`.md`、`.dot`、`.epub`、`.html`、`.json`、`.mobi`、`.log`、`.go`、`.h`、`.c`、`.cpp`、`.cxx`、`.cc`、`.cs`、`.java`、`.js`、`.css`、`.jsp`、`.php`、`.py`、`.py3`、`.asp`、`.yaml`、`.yml`、`.ini`、`.conf`、`.ts`、`.tsx` 等。

  注意：图片文件不再支持内容抽取（`file-extract`）。如需图片理解，请使用 `purpose="image"` 上传图片，详见[使用视觉模型](/docs/guide/use-kimi-vision-model)。
</Accordion>
...
<Accordion title="文件内容抽取示例">
  ...
  <Tabs>
    <Tab title="python">
      ...
      # xlnet.pdf 是一个示例文件, 我们支持 pdf, doc 等文本类格式
...
paths:
  /v1/files:
    post:
      ...
      summary: 上传文件
      description: >-
        以 multipart 表单数据上传一个文件，并返回其文件记录。


        - **表单**：`file` 部分必填，须携带文件名（或在表单中显式提供 `filename`）。该部分自身的 Content-Type
        可以省略，存储的 MIME 类型将按文件内容与文件名检测。

        - **限制**：单个文件上限 100 MiB 且不能为空。上传后的文件不可修改。

        - **命名**：与现有文件重名上传时，服务器会在存储的文件名上添加后缀，例如 `report (1).txt`。
...
components:
  ...
  schemas:
    ...
    UploadFilename:
      type: string
      maxLength: 4096
      description: 上传时显式指定的文件名，覆盖文件部分自身的名称。
    FilePurpose:
      type: string
      enum:
        - file-extract
