...
MoonPalace 提供了对 Kimi 大模型重复内容输出的检测功能。重复内容输出指的是：\*\*Kimi 大模型会重复不断地输出某一特定字词、句子以及空白字符，并且在达到 `max_tokens` 限制前不会停下来。\*\*在使用上下文较长、费用较高的模型时，这种重复输出会导致额外的 Tokens 费用消耗，因此 MoonPalace 提供了 `--detect-repeat` 选项以启用重复内容输出检测，如下所示：
