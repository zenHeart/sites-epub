...
例如批处理的处理对象是一个长度为 50 的数组，设置**并行运行数量上限**为 10，设置**批处理次数上限**为 30，那么批处理节点只会执行 3 批，每一批同时运行 10 次，共计执行 30 次后就结束运行。
...
![Image=1452x658](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/6373bbc52ecd4ddfb2dbc798d9c4019d~tplv-goo7wpa0wc-topic.webp)

工作流核心节点如下：

<!-- @cols-width: 142,434,263 -->
| 节点类型  | 说明  | 示例  |
| --- | --- | --- |
| 开始节点  | 定义两个输入参数： | ![Image=600x445](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/88c0cd4a50d344459296281c97c6aeea~tplv-goo7wpa0wc-topic.webp)  | \
| | | | \
| | * image：图片数组格式，支持上传多张图片。 | | \
| | * query：String 格式，后续节点将作为模型的提示词。  | |
| 批处理节点  | 设置输入参数 image，引用开始节点的 image 参数，便于批处理体引用。  | ![Image=600x336](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/03243f528701435ba1802b405968792e~tplv-goo7wpa0wc-topic.webp)  |
| 批处理体  | 添加一个大模型节点，其设置如下： | ![Image=600x448](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/9ce858ed4b9a4f659a8c61952591dd47~tplv-goo7wpa0wc-topic.webp)  | \
| | | | \
...
批处理节点会按照设定的并行数量分批运行批处理体，直到处理的数组中所有元素处理完成，或达到指定的次数上限。你可以调整待处理的数组长度，或者通过**批处理次数上限**来限制批处理节点的最大运行次数，后者最大值为 200。

### 批处理节点和循环节点有什么区别 {#ba62374b}
