...
<tr>
<td>标签</td>
<td>配置是否显示单行输入组件的标签，标签可以为单行输入组件提供文字描述，使用户能够更直观地理解单行输入组件的状态或功能。</td>
</tr>
<tr>
<td>
<p>标签内容</p>
...
<td>
<p>配置单行输入组件的标签内容，支持配置静态内容和动态绑定数据，详情请参见<a href="/guides/set_content_parameters" target="_blank">设置组件内容参数</a>。</p>
<p>仅打开了单行输入组件的设置开关时，才支持此配置项。默认标签内容为<strong>请输入文本</strong>。</p>
</td>
</tr>
<tr>
<td>
...
<td>
<p>配置单行输入的占位文案，占位文案是一种提示性文本，它出现在输入字段中但不会显示在最终提交的数据里。</p>
<p>默认为<strong>请输入内容</strong>。</p>
</td>
</tr>
<tr>
<td>字符上限</td>
<td>配置单行输入组件的字符上限，字符上限是一个限制用户在单行输入框中可以输入的最大字符数的功能。默认关闭字符上限，你可以打开开关，并根据实际需求设置字符上限数值。</td>
</tr>
<tr>
<td>默认值</td>
<td>配置单行输入组件的默认值。</td>
...
<tr>
<td>组件布局</td>
<td>配置标签与输入框的布局方式，支持上下布局和左右布局。</td>
</tr>
<tr>
<td>
<p>垂直对齐</p>
...
<tr>
<td>输入区颜色</td>
<td>配置单行输入框背景的显示颜色。</td>
</tr>
<tr>
<td>输入区边框</td>
<td>配置单行输入组件背景的边框颜色、边框的粗细和边框的样式。</td>
</tr>
</tbody>
</table>
</div><h3 id="a8fb7c0d" tabindex="-1">位置设置</h3>
<p>单行输入组件提供三种位置类型选项，以灵活配置组件在界面中的位置。</p>
<ul data-style="0">
...
<td>
<p>配置单行输入组件的尺寸限制，支持配置最大高度、最小高度、最大宽度、最小宽度。</p>
<p>合理设置尺寸限制参数可以确保组件在不同屏幕尺寸下保持布局的稳定性和一致性，避免因屏幕尺寸差异而导致的布局问题。</p>
</td>
</tr>
</tbody>
</table>
...
<tr>
<td>填充</td>
<td>配置单行输入组件背景的显示颜色。</td>
</tr>
<tr>
<td>圆角</td>
<td>配置单行输入组件背景的圆角，支持单独设置四个角的圆角。</td>
</tr>
<tr>
<td>内边距</td>
<td>配置单行输入组件背景的内边距，可以单独设置四个方向的内边距。</td>
</tr>
<tr>
<td>外边距</td>
<td>配置单行输入组件背景的外边距，可以单独设置四个方向的外边距。</td>
</tr>
<tr>
<td>
<p>边框</p>
...
<td>
<p>配置单行输入组件背景的阴影样式，增强组件的立体感。支持如下设置方式：</p>
<ul data-style="0">
...
<tr>
<td>图层模糊</td>
<td>配置单行输入组件的模糊效果。数值越大，模糊效果越明显。</td>
</tr>
<tr>
<td>
<p>溢出</p>
...
</div><h3 id="b0d5c4e3" tabindex="-1">变换设置</h3>
<p>丰富界面视觉效果的参数。</p>
<!-- @cols-width: 138,707 -->
<div class="topic-table-container">
<table class="topic-table-fixed">
...
<tr>
<td>缩放</td>
<td>配置单行输入组件大小的缩放比例。</td>
</tr>
<tr>
<td>旋转</td>
<td>配置单行输入组件围绕中心点旋转的角度。</td>
</tr>
<tr>
<td>位移</td>
<td>配置单行输入组件在水平和垂直方向上的移动距离。</td>
</tr>
<tr>
<td>倾斜</td>
<td>配置单行输入组件在水平和垂直方向上的倾斜角度。</td>
</tr>
</tbody>
</table>
</div><h3 id="54822e78" tabindex="-1">状态设置</h3>
<p>控制单行输入组件禁用态、加载态、点击态和出现态的视觉效果参数。</p>
<!-- @cols-width: 140,712 -->
<div class="topic-table-container">
<table class="topic-table-fixed">
<colgroup><col style="width: 140px;" /><col style="width: 712px;" /></colgroup><thead>
...
<td>
<p>配置单行输入组件的禁用状态。具体配置说明如下：</p>
<ul data-style="0">
<li>设置为 true：组件处于禁用状态。</li>
<li>设置为 false：组件处于可用状态。</li>
<li>设置为变量：通过变量值（true 或 false）动态控制单行输入组件的禁用状态。<br>
例如可以通过工作流控制单行输入组件的禁用状态。工作流为 <code>example_workflow</code>，当你将单行输入组件<strong>禁用态</strong>设置为<code>{{example_workflow.disable.value}}</code>后，当工作流的 <code>disable</code> 属性为 <code>true</code> 时，<code>{{example_workflow.disable.value}}</code> 值为 true，单行输入组件将处于禁用状态。</li>
</ul>
</td>
</tr>
<tr>
...
<td>
<p>配置单行输入组件的加载状态。具体配置说明如下：</p>
<ul data-style="0">
<li>设置为 true：组件展示加载状态，隐藏组件内容。</li>
<li>设置为 false：关闭加载状态。</li>
<li>设置为变量：通过变量值（true 或 false）动态控制单行输入组件的加载状态。<br>
例如可以通过单行输入组件控制单行输入组件的加载状态。单行输入组件 ID 为 <code>Input1</code>，当你将单行输入组件的<strong>加载态</strong>设置为<code>{{Input1.value}}</code>后，在单行输入组件中输入文本时，<code>{{Input1.value}}</code>值为 true，单行输入组件展示加载态。</li>
</ul>
</td>
</tr>
<tr>
...
<td>
<p>配置单行输入组件的点击态效果。具体配置说明如下：</p>
<ul data-style="0">
<li>样式效果：点击单行输入组件时，单行输入组件的样式变化情况。详细说明，请参考<a href="/guides/single_line_input#e4a081a0" target="_blank">样式设置</a>。</li>
<li>变换效果：点击单行输入组件时，单行输入组件的动态变化情况。详细说明，请参考<a href="/guides/single_line_input#b0d5c4e3" target="_blank">变换设置</a>。</li>
<li>过渡效果：设置单行输入组件点击态的动画效果。
<ul data-style="1">
...
<tr>
<td>出场效果</td>
<td>设置单行输入组件在界面中出现时的动画效果。目前支持设置为淡入、缩放、模糊、水平翻转、垂直翻转、从下滑入、从上滑入、从左滑入、从右滑入等动画效果。</td>
</tr>
</tbody>
</table>
</div><h3 id="9766f9c6" tabindex="-1"><strong>可见性设置</strong></h3>
<p>控制单行输入组件隐藏或显示的参数，支持在特定条件下隐藏单行输入组件。</p>
<ul data-style="0">
<li>设置为 false：显示组件。</li>
<li>设置为 true：隐藏组件。</li>
<li>设置为变量：通过变量值（true 或 false）动态控制单行输入组件的可见性。<br>
例如可以通过单行输入组件控制单行输入组件的可见性。单行输入组件 ID 为 <code>Input1</code>，当你将单行输入组件的<strong>可见性</strong>设置为<code>{{ Input1.value}}</code>后，在单行输入组件中输入文本时，<code>{{ Input1.value}}</code>值为 true，单行输入组件将被隐藏。</li>
</ul>
<h2 id="27f58bb8" tabindex="-1">事件设置</h2>
<p>通过配置单行输入组件的事件，可以为单行输入组件添加丰富的交互功能，以增强用户体验和界面的互动性。</p>
<!-- @cols-width: 148,709 -->
<div class="topic-table-container">
<table class="topic-table-fixed">
<colgroup><col style="width: 148px;" /><col style="width: 709px;" /></colgroup><thead>
<tr>
<th><strong>事件项</strong></th>
<th><strong>说明</strong></th>
</tr>
</thead>
<tbody>
