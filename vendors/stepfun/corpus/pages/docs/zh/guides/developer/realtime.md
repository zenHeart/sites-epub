...
服务器发出的事件完整列表见[Realtime API 事件列表](/docs/zh/api-reference/realtime/chat)，它们按大致的发出顺序排列，同时包含文本生成相关的客户端事件。
...
| 请求头  | Authorization: Bearer YOUR\_STEP\_API\_KEY                           |
...
> `step-audio-2` 模型支持音色复刻功能。您可以通过上传音频文件创建自定义音色，并在实时会话的 `voice` 参数中使用其 ID。详情请参考[复刻音色](/docs/zh/api-reference/audio/create-voice)。
