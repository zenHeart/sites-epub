...
The headless daemon is built into the [Antigravity CLI](/docs/cli/install).

1.  [Install the CLI](/docs/cli/install) and sign in once: run `agy`, complete the sign-in flow, then exit.
...
Run `agy remote-control status` to see this machine’s instance name, then open the [Antigravity Remote Control Dashboard](https://antigravity.google.com) to find it.
